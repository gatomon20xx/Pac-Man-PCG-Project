...
public class Mapgen
{
    ...
    public static void genRandom()
    {
        ...
        void reassignGroup(object oldg,object newg)
        {
            Cell c = null;
            for (int i=0; i<rows*cols; i++) {
                c = cells[i];
        ...
        bool createTunnels()
        {
            ...
            void replaceGroup(object oldg, object newg) {
                Cell c = null;
                for (int i=0; i<rows*cols; i++) {
                    c = cells[i];
                    if (c.group == oldg) {
    ...
    public static string getTiles()
    {
        ...
        for (int y4=0; y4<subrows; y4++) {
            for (int x1=0; x1<subcols; x1++) {
                c = getTileCell(x1,y4); // cell
                cl = getTileCell(x1-1,y4); // left cell
                cu = getTileCell(x1,y4-1); // up cell
                ...
                if (c != null) {
                    ...
                    // outside map
                    if ((cl != null && (!cl.connect[RIGHT] || getTile(x1-1,y4) == '.')) || // at right boundary
                        (cu != null && (!cu.connect[DOWN] || getTile(x1,y4-1) == '.'))) { // at bottom boundary
                        setTile(x1,y4,'.');
                    }
                }

                // at corner connecting two paths
                if (getTile(x1-1,y4) == '.' && getTile(x1,y4-1) == '.' && getTile(x1-1,y4-1) == '_') {
                    setTile(x1,y4,'.');
                }
            }
        }
        ...
        for (c=cells[cols-1]; c != null; c = c.next[DOWN]) {
            if (c.topTunnel) {
                y5 = c.final_y+1;
                setTile(subcols-1, y5,'.');
                setTile(subcols-2, y5,'.');
            }
        }

        // fill in walls
        for (int y1=0; y1<subrows; y1++) {
            for (int x1=0; x1<subcols; x1++) {
                // any blank tile that shares a vertex with a path tile should be a wall tile
                if (getTile(x1,y1) != '.' && (getTile(x1-1,y1) == '.' || getTile(x1,y1-1) == '.' || getTile(x1+1,y1) == '.' || getTile(x1,y1+1) == '.' ||
                    getTile(x1-1,y1-1) == '.' || getTile(x1+1,y1-1) == '.' || getTile(x1+1,y1+1) == '.' || getTile(x1-1,y1+1) == '.')) {
                    setTile(x1,y1,'|');
                }
            }
        }
        ...
        if(y2 != 0)
        {
            y = getRandomInt(range.Item1, range.Item2);
            setTile(x2,y2,'o');
        }
